...
for model in clustered_models:
    if '.caffemodel' in model:
        ...
        if prune_type == 'layer_indep':
            cmd = 'python {} --prune_type={} --prune_percent={} --centroids={} --clustered_model={}'.format(retraining_script, 'layer_indep', prune_percent, centroids, clustered_model)
        else:
            cmd = 'python {} --prune_type={} --prune_percent={} --std_dev={} --centroids={} --clustered_model={}'.format(retraining_script, 'layer_wise', prune_percent, std_dev, centroids, clustered_model)
