...
usage = '''
Usage:
        python retrain_pruned_ssd_pascal_resnet.py <prune-tpye> <prune-percent> <pruned-model>
                                or
        python retrain_pruned_ssd_pascal_resnet.py <prune-type> <prune-percent> <std-dev> <pruned-model>
...
def AddExtraLayers(net, use_batchnorm=True, prune_mask=False, cluster_mask=False):
    use_relu = True
    ...
    ConvBNLayer(net, from_layer, out_layer, use_batchnorm, use_relu, 256, 1, 0, 1, prune_mask=prune_mask, cluster_mask=cluster_mask)
    from_layer = out_layer

    out_layer = "{}/conv1_2".format(last_layer)
    ConvBNLayer(net, from_layer, out_layer, use_batchnorm, use_relu, 512, 3, 1, 2, prune_mask=prune_mask, cluster_mask=cluster_mask)
    from_layer = out_layer
...
output_result_dir = "{}/workspace/Datasets/VOCdevkit/results/VOC2007/{}/Main".format(os.environ['HOME'], job_name)
...
ResNet101Body(net, from_layer='data', use_pool5=False, use_dilation_conv5=True, cluster_mask=True)

# Use batch norm for the newly added layers.
AddExtraLayers(net, use_batchnorm=True, cluster_mask=True)
...
mbox_layers = CreateMultiBoxHead(net, data_layer='data', from_layers=mbox_source_layers,
        ...
        flip=flip, clip=clip, prior_variance=prior_variance, kernel_size=3, pad=1, cluster_mask=True)
