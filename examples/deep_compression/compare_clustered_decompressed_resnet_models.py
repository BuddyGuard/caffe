    ...
    for name, param in clustered_net.params.iteritems():
        for pos, p in enumerate(param):
            if len(p.data.shape) == 4:
                ...
                clustered_params =  p.data
                decompressed_params = decompressed_net.params[name][pos].data
                total_params_diff += np.sum(clustered_params) - np.sum(decompressed_params)
    models_diff[clustered_model] = {'decompressed_model':decompressed_model,
                                    'weights_diff':total_weights_diff,
                                    'params_diff':total_params_diff}
...
for key in models_diff.keys():
    ...
    print 'Other params diff  : {}'.format(models_diff[key]['params_diff'])
