...
def ConvBNLayer(net, from_layer, out_layer, use_bn, use_relu, num_output,
    kernel_size, pad, stride, dilation=1, use_scale=True, prune_mask=False, cluster_mask=False, eps=0.001,
  ...
  if use_bn:
    ...
    if prune_mask:
      kwargs = {'param': [dict(lr_mult=1, decay_mult=1)],
                'pruning_param': dict(fill_prune_mask=True),
                'weight_filler': dict(type='gaussian', std=0.01),
                'bias_term': False,
        }
    elif cluster_mask:
      kwargs = {'param': [dict(lr_mult=1, decay_mult=1)],
                'clustering_param': dict(fill_cluster_mask=True),
                'weight_filler': dict(type='gaussian', std=0.01),
                'bias_term': False,
        }
    else:
...
def ResBody(net, from_layer, block_name, out2a, out2b, out2c, stride, use_branch1, dilation=1, prune_mask=False, cluster_mask=False):
  ...
  if use_branch1:
    ...
    ConvBNLayer(net, from_layer, branch_name, use_bn=True, use_relu=False,
        ...
        scale_prefix=scale_prefix, scale_postfix=scale_postfix, prune_mask=prune_mask, cluster_mask=cluster_mask)
...
def VGGNetBody(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
        dilated=False, nopool=False, dropout=True, prune_mask=False, cluster_mask=False, freeze_layers=[]):
...
def ResNet101Body(net, from_layer, use_pool5=True, use_dilation_conv5=False, prune_mask=False, cluster_mask=False):
    conv_prefix = ''
    conv_postfix = ''
    bn_prefix = 'bn_'
    bn_postfix = ''
    ...
    ResBody(net, 'pool1', '2a', out2a=64, out2b=64, out2c=256, stride=1, use_branch1=True, prune_mask=prune_mask, cluster_mask=cluster_mask)
    ResBody(net, 'res2a', '2b', out2a=64, out2b=64, out2c=256, stride=1, use_branch1=False, prune_mask=prune_mask, cluster_mask=cluster_mask)
    ResBody(net, 'res2b', '2c', out2a=64, out2b=64, out2c=256, stride=1, use_branch1=False, prune_mask=prune_mask, cluster_mask=cluster_mask)

    ResBody(net, 'res2c', '3a', out2a=128, out2b=128, out2c=512, stride=2, use_branch1=True, prune_mask=prune_mask, cluster_mask=cluster_mask)

    from_layer = 'res3a'
    for i in xrange(1, 4):
      block_name = '3b{}'.format(i)
      ResBody(net, from_layer, block_name, out2a=128, out2b=128, out2c=512, stride=1, use_branch1=False, prune_mask=prune_mask, cluster_mask=cluster_mask)
      from_layer = 'res{}'.format(block_name)

    ResBody(net, from_layer, '4a', out2a=256, out2b=256, out2c=1024, stride=2, use_branch1=True, prune_mask=prune_mask, cluster_mask=cluster_mask)

    from_layer = 'res4a'
    for i in xrange(1, 23):
      block_name = '4b{}'.format(i)
      ResBody(net, from_layer, block_name, out2a=256, out2b=256, out2c=1024, stride=1, use_branch1=False, prune_mask=prune_mask, cluster_mask=cluster_mask)
      from_layer = 'res{}'.format(block_name)
    ...
    ResBody(net, from_layer, '5a', out2a=512, out2b=512, out2c=2048, stride=stride, use_branch1=True, dilation=dilation, prune_mask=prune_mask, cluster_mask=cluster_mask)
    ResBody(net, 'res5a', '5b', out2a=512, out2b=512, out2c=2048, stride=1, use_branch1=False, dilation=dilation, prune_mask=prune_mask, cluster_mask=cluster_mask)
    ResBody(net, 'res5b', '5c', out2a=512, out2b=512, out2c=2048, stride=1, use_branch1=False, dilation=dilation, prune_mask=prune_mask, cluster_mask=cluster_mask)
...
def CreateMultiBoxHead(net, data_layer="data", num_classes=[], from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, prune_mask=False, cluster_mask=False,
    ...
    for i in range(0, num):
        ...
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, 
                    prune_mask=prune_mask, cluster_mask=cluster_mask, num_output=num_loc_output, 
        ...
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False,
            prune_mask=prune_mask, cluster_mask=cluster_mask, num_output=num_conf_output, 
            kernel_size=kernel_size, pad=pad, stride=1)
        permute_name = "{}_perm".format(name)
        net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
        flatten_name = "{}_flat".format(name)
